...
public class DataDrivenGetTest {
	...
	public Object[][] getPostIds() {
		return new Object[][] { { 1, 1 }, // postId, expectedUserId
				{ 2, 1 }, { 3, 1 }, { 4, 1 }, { 5, 1 } };
	}
	...
	public void testMultiplePostsTraditional(int postId, int expectedUserId) {
		...
		Response response = given().header("Accept", "application/json").pathParam("id", postId).get("/posts/{id}");
	...
	public void testMultiplePostsBDD(int postId, int expectedUserId) {
		...
		given().pathParam("id", postId).header("Accept", "application/json").when().get("/posts/{id}").then()
				.statusCode(200).contentType("application/json").body("id", equalTo(postId))
				.body("userId", equalTo(expectedUserId)).body("title", notNullValue()).time(lessThan(3000L)).log()
				.ifValidationFails();
	}
}